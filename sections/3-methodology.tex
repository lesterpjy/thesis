...
drive $S\to0$. Under a Lipschitz bound $|\nabla_{\xi_t}\log r_\ell|\le C$ on the
log-likelihood-ratio surface, the triangle inequality gives
$\big|\sum_\ell r_\ell\nabla_{\xi_t}\log r_\ell\big|\le CS$, hence
\begin{equation}
\label{eq:grad-bound}
|\nabla_{\xi_t}\mathrm{sPCE}_L|\le\frac{CS}{1+S}\xrightarrow{S\to0}0.
\end{equation}
Even without the Lipschitz bound, every term in the numerator carries the factor
$r_\ell$, which is exponentially small. This is stronger than value saturation: not
only does every design score about $\log(L+1)$, but the gradient is near zero
everywhere, so the policy receives no direction and its parameters drift on Monte
Carlo noise. Both the objective and its optimisation fail at once.
...
quadratic in the predicted mean. Concentration data are usually modelled with
lognormal noise, for which the log-likelihood gap becomes, in expectation over
$y\sim p(y\mid\theta_0,\xi)$,
$\mathbb{E}_y[\log r_\ell]=-\frac{1}{2\sigma^2}\sum_k(\log\mu^{(k)}(\theta_\ell)-\log\mu^{(k)}(\theta_0))^2$,
a sum over output channels of squared log-differences. Unlike Gaussian noise, which
is dominated by the largest-magnitude species, lognormal noise penalises a mismatch
at every channel equally in relative terms, so on systems whose species span orders
of magnitude the penalties accumulate. Heteroscedastic noise with a scale that
shrinks at low means is worse still: a contrastive predicting a different order of
magnitude has a narrow noise scale, so the squared residual against the observed
$y\approx\mu(\theta_0)$ diverges. In both cases the gap grows faster than the
Gaussian-quadratic prediction. The three mechanisms compound: broad priors make the
displacement $\|\theta_\ell-\theta_0\|$ large, high-dynamic-range maps turn that into
a large observation-space displacement, and non-Gaussian noise turns that into a more
negative $\log r_\ell$ than the quadratic approximation predicts. Systems with
several of them, such as ASM1 and SEIR, show the largest gap between $e^{\Lambda}$ and
$L_{\mathrm{escape},50}$.
