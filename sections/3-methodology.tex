...
The derivations are given in full. A few proofs are standard and are stated in the
text with the full version in Appendix~\ref{app:background}; everything specific to
PACE is derived here.
...
Equivalently, $I_{h_{t-1}}(\xi_t)=H[p(\theta\mid h_{t-1})]-\mathbb{E}_{y_t}H[p(\theta\mid h_t)]$,
the expected drop in posterior entropy from observing $y_t$. The key feature for
...
deployment~\citep{hedman2025stepdad}, by reinforcement-learning and value-based
variants~\citep{blau2022optimizing,belliardo2024modelaware}, by amortized variants
...
the expected log of an unbiased estimate underestimates the log of the mean, the
marginal is underestimated, the ratio overestimated, and
...
The estimator in Eq.~\eqref{eq:pace} belongs to a lineage of contrastive EIG
estimators from the Foster and Rainforth line of work, and being precise about what is
and is not new in PACE matters for its positioning. \citet{foster2019variational}
introduced the variational approach to EIG estimation, learning the inner inference
network once and reusing it across outcomes, and gave the posterior-style lower bound
and the nested-Monte-Carlo upper bound that PACE's evaluation brackets descend from.
\citet{foster2020unified} named the two estimators we build on. Prior contrastive
estimation (PCE) draws the contrastives from the prior; adaptive contrastive
estimation (ACE) draws them from a \emph{learned} proposal $q_\phi(\theta\mid y)$ and
adds the data-generating $\theta_0$ to the denominator, which turns the estimator into
a lower bound that can be optimised jointly in the design and the proposal. Their
Theorem~1 records the property PACE reuses: if the proposal equals the posterior, the
estimator is exact for \emph{every} contrastive count, with no contrastives needed.
This is the static analogue of our perfect-proposal identity
(Section~\ref{sec:m-costtransfer}). Crucially, ACE in \citet{foster2020unified} is a
\emph{static}, single-experiment estimator: the proposal conditions on one outcome
and is trained jointly with the design.

The sequential, learned-proposal versions exist only as unevaluated appendix results.
\citet{foster2021deep} state and prove the sequential ACE bound (their Appendix~B,
Theorem~5) but train DAD on prior-contrastive sPCE, explicitly deferring the learned
proposal ``in the interest of simplicity''. \citet{ivanova2021implicit} give the
likelihood-free version, sLACE (their Appendix~B, Proposition~4), with a training
recipe, but their experiments use its prior-proposal special case (InfoNCE). So the
sequential learned-proposal estimator was foreseen, but neither paper implements or
evaluates it.

Written explicitly, the sequential ACE bound \citep[Theorem~5, Eq.~91]{foster2021deep} is
\begin{equation}
\label{eq:sace}
...
\end{equation}
and its likelihood-free analogue sLACE \citep[Proposition~4, Eq.~72]{ivanova2021implicit}
replaces the likelihood $p(h_T\mid\theta_\ell,\pi)$ with a learned critic
$U(h_T,\theta_\ell)$,
\begin{equation}
\label{eq:slace}
I_T(\pi) \ge \mathbb{E}_{p(\theta_0,h_T\mid\pi)\,q(\theta_{1:L};h_T)}
...
\end{equation}
Two features of Eqs.~\eqref{eq:sace}--\eqref{eq:slace} matter for the comparison. The
proposal $q(\theta;h_T)$ conditions on the \emph{full trajectory}, and each contrastive
enters the self-normalised denominator with weight $p(\theta_\ell)/q(\theta_\ell;h_T)$,
so the denominator is an importance-sampling estimate of the prior marginal
$\int p(h_T\mid\theta,\pi)\,p(\theta)\,\ud\theta=p(h_T\mid\pi)$, which is the denominator
of the \emph{trajectory} mutual information. Setting $q=p(\theta)$ recovers prior
contrastives (sPCE, and InfoNCE in the likelihood-free case) and the $\log(L+1)$
ceiling. PACE (Eq.~\eqref{eq:pace}) keeps this algebraic shape but makes two changes:
the proposal conditions on the history prefix $h_{t-1}$, and the weight is
...
rather than the prior marginal. The three differences below make this precise.

PACE differs from these appendix bounds in three ways, none of which is in the
algebra they share.
...
\end{equation}
and declines as the posterior tightens. That this $\chi^2$ is finite at all, which the
...
competitors, with fixed weights $w_k/w_{\mathrm{dom}}$.
...
and treating $\theta_{k^\ast}$ as a posterior draw with covariance
$\Sigma_{\mathrm{post}}$ gives the expected criterion
$\tfrac12\,\mathrm{tr}\!\big(F(\xi_t)\Sigma_{\mathrm{post}}\big)$, Fisher information
weighted by residual posterior uncertainty. This is the trace form of the
Gaussian-linearised EIG, so the quantity $\Lambda$ that drove saturation in
Section~\ref{sec:m-saturation} reappears here as the design objective.
...
at the same $L$, which bounds the true mean EIG from below and above. When one
method's lower bound exceeds another's upper bound by more than the combined standard
error, the ordering is definitive and no estimation slack can reverse it. All main
